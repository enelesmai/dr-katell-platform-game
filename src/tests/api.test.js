...
test('expect getScore service returns array of objects', () => {
    api.getScore()
        .then((response) => {
            expect(response[0]).toEqual(expect.objectContaining({ score: expect.any(Number), name: expect.any(String) }));
        });
});
...
test('expect getScore service returns sorted objects by score', () => {
    api.getScore()
        .then((response) => {
            expect(response).toEqual(expect.arrayContaining(expectedsort((a, b) => b.score - a.score)))
        });
});
